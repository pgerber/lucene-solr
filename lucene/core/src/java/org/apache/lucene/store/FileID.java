 ...
 * very that both are the same file. This comparison is done on an best-effort basis and is
 * useful to ensure that a file has not been removed and recreated.</p>
 *
 * <p>Implementation details:</p>
 *
 * <ul>
 *   <li>On plattforms that support {@code BasicFileAttributes#fileKey} this will be used to verify that
 *   two {@code FileID}s refer to same file. On Unix-like systems, the key consists of the <em>device id</em>
 *   and <em>inode</em> allowing detection of recreated files at a very low error rate.</li>
 *   <li>On all other platforms, namely those that don't support {@code BasicFileAttributes#fileKey}, the
 *   creation time or modification time, if available, is used in an attempt to ensure two {@code FileID}s
